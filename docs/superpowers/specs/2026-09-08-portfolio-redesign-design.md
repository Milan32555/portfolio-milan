...
> Sesión de brainstorming iniciada 2026-09-08. **Actualizado 2026-09-22**: `/servicios`, el footer "Cordilleras" y `/proyectos` ya están cerrados **e implementados en producción** (ver puntos 1, 2 y 4 de Pendiente). **Actualizado 2026-09-22 (noche)**: `/sobre-mi` y el home resumido con un hero nuevo en Three.js ya están diseñados y aprobados (specs del 2026-09-22; el Loader pasa a ser un "muro de código" interactivo). Falta implementarlos — y el rediseño grande del sitio (sidebar fijo + paleta navy + cursor "resplandor etéreo") **todavía no se ha implementado**: lo ya construido (footer, `/proyectos`) vive sobre el diseño viejo actual (navbar superior, `--bg:#0c0c0e`, tema claro/oscuro), no sobre la navy nueva. **Actualizado 2026-09-24**: home + hero Three.js + muro de código, `/sobre-mi`, `/contacto` y `/servicios` están **implementados y en producción** (PRs #10 y #12–#16). Lo único grande que falta es el rediseño sidebar + navy + cursor glow (y la decisión del grano). Retomar leyendo este archivo primero.
...
1. ~~`/servicios`~~ — **IMPLEMENTADO Y EN PRODUCCIÓN 2026-09-24** (PR #13, plan `docs/superpowers/plans/2026-09-23-pagina-servicios.md`), adaptado al diseño actual: la navbar superior se mantiene y el TOC es una columna sticky del artículo (no hay sidebar que reemplazar todavía), sin Framer Motion (usa `Reveal`), copy tuteado, sin montos (test), y sin stack fijo en las líneas de desarrollo (PR #16: el stack depende de cada proyecto). Spec de diseño cerrado 2026-09-17: `docs/superpowers/specs/2026-09-17-pagina-servicios-design.md`. TOC puro reemplaza al sidebar en esta página (no conviven), estructura editorial con 6 secciones + FAQ acordeón, precios sin montos públicos. Ese spec también documenta 4 correcciones de accesibilidad/mobile sitewide encontradas durante la investigación (ver punto 7 abajo).

2. ~~`/proyectos`~~ — **IMPLEMENTADO Y EN PRODUCCIÓN 2026-09-22**, ver `docs/superpowers/specs/2026-09-21-pagina-proyectos-design.md` (diseño) y `docs/superpowers/plans/2026-09-21-pagina-proyectos.md` (plan de implementación, ejecutado con subagentes). `/proyectos` (lista) y `/proyectos/[slug]` (detalle, 4 proyectos) están en vivo: sub-página por proyecto con botones "Ver demo"/"Ver código", lista con miniatura + cifra verificable + tags, detalle con cifras destacadas, arquitectura explicada en lenguaje simple y el componente Expediente (carpetas del punto 10). Construido sobre los tokens **actuales** del sitio (CSS Modules + `.btn-primary`/`.tag`/`.section` existentes), no sobre la navy del rediseño grande. Contenido confirmado por el usuario (rol, decisiones, periodo/estado reales por fecha de creación del repo en GitHub); capturas reales solo para AnimalVision y library-system (las que tienen demo desplegada). PRs #6 y #7 (`Milan32555/portfolio-milan`), fusionados a `main`.

3. ~~`/sobre-mi`~~ — **DISEÑADO Y APROBADO 2026-09-22**, ver `docs/superpowers/specs/2026-09-22-pagina-sobre-mi-design.md`: titular en DM Serif + consola estilo Kali que se escribe sola (whoami, stack, status con la app Base, fuera del código) y comandos bonus `neofetch`/`history`. **Implementado y en producción 2026-09-23** (PR #10).

4. ~~Home resumido~~ — **DISEÑADO Y APROBADO 2026-09-22** junto con un hero nuevo, ver `docs/superpowers/specs/2026-09-22-home-y-hero-design.md`: hero con "Misael." hecho de código en Three.js (escaneo de auditoría con hallazgos, easter egg `whoami`) + 4 secciones resumidas (qué hago, 3 proyectos, avance de la consola, CTA de contacto). **Implementado y en producción 2026-09-23** (PR #10). En Android real el 3D caía a texto: la causa fue `document.fonts.load` con el respaldo `local()` de next/font (no existe en Android); corregido en PRs #15/#16. Queda un modo de diagnóstico `?diag` en el hero para depurar en teléfonos reales.
...
7. **Correcciones sitewide encontradas investigando `/servicios`** (no estaban contempladas el 2026-09-08, deben entrar al plan de implementación general, detalle en `docs/superpowers/specs/2026-09-17-pagina-servicios-design.md`):
   ...
   - El nuevo cursor "resplandor etéreo" (sección 3) debe gatear (el cursor ACTUAL ya lo hace desde el PR #12: solo con mouse fino y sin estela con movimiento reducido; el nuevo debe conservarlo) por `(hover: hover) and (pointer: fine)` y respetar `prefers-reduced-motion` explícitamente en su loop de `requestAnimationFrame` — el `prefers-reduced-motion` CSS del proyecto no cubre animación JS pura.
...
9. **Three.js — nuevo alcance, no estaba en la sesión original del 2026-09-08**: el usuario pidió incorporar Three.js "para algo". Se decidió explícitamente que **NO va en `/servicios`** (compite con la lectura larga) sino en el **Loader** (`components/Loader.tsx`) — pendiente de su propia sesión de brainstorming completa (no diseñado todavía). **Actualizado 2026-09-22: cambió.** El usuario lo movió al **hero** del home (visible en cada visita). El Loader se mantiene, rediseñado como "muro de código" interactivo (una vez por sesión, clic/Enter para entrar, se abre en dos y el código sale de la rendija para formar el nombre del hero). Ver `docs/superpowers/specs/2026-09-22-home-y-hero-design.md`. La investigación (pasada 3, GitHub) recomienda una escena 3D **contenida** (ej. estilo `sunnypatell/react-threejs-portfolio`), no un mundo navegable completo (ej. Bruno Simon / `VinayMatta63/threejs-portfolio`), para no repetir ese patrón fuera de contexto.

10. ~~Componente "Expediente" (carpetas apiladas)~~ — **IMPLEMENTADO 2026-09-22** en el detalle de cada proyecto (`/proyectos/[slug]`, componente `components/proyectos/Expediente.tsx`): 5 carpetas (Problema abierta por defecto, Mi rol, Decisiones, Evidencia, Resultado), se abren con clic, teclado o hover con intención; el resumen y los botones demo/código quedan siempre visibles fuera de las carpetas, así que nada queda oculto para quien escanea rápido.
