...
3. ~~`/sobre-mi`~~ — **DISEÑADO Y APROBADO 2026-09-22**, ver `docs/superpowers/specs/2026-09-22-pagina-sobre-mi-design.md`: titular en DM Serif + consola estilo Kali que se escribe sola (whoami, stack, status con la app Base, fuera del código) y comandos bonus `neofetch`/`history`. Aún no implementado.

4. ~~Home resumido~~ — **DISEÑADO Y APROBADO 2026-09-22** junto con un hero nuevo, ver `docs/superpowers/specs/2026-09-22-home-y-hero-design.md`: hero con "Misael." hecho de código en Three.js (escaneo de auditoría con hallazgos, easter egg `whoami`) + 4 secciones resumidas (qué hago, 3 proyectos, avance de la consola, CTA de contacto). Aún no implementado.
